...
export default function FacturaComercial() {
  ...
  useEffect(() => {
    ...
    if (!factActual) {
      ...
      factActual = `FACT-${año}-${Math.floor(1000 + Math.random() * 9000)}`;
    }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    doc.save(`Factura_Comercial_${numeroFactura}.pdf`);
  };
    ...
    <main className="min-h-screen bg-gray-100 p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-5xl mb-4 flex justify-between items-center">
        <button 
          onClick={() => router.back()} 
          type="button"
          className="bg-white text-blue-600 border border-blue-600 px-4 py-2 rounded font-bold hover:bg-blue-50 transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-8 border-t-8 border-blue-500">
        
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 mb-8 rounded-r-lg">
          <h3 className="font-bold text-blue-800 text-lg mb-1">Paso 8: Factura Comercial Final</h3>
          <p className="text-blue-700 text-sm">
            <strong>Tu Rol:</strong> Exportador / Vendedor.<br/>
            <strong>Objetivo:</strong> Emitir el documento financiero definitivo que ampara la mercancía y sirve para la nacionalización en destino.
          </p>
        </div>

        <h2 className="text-3xl font-bold text-center text-blue-600 mb-8">Factura Comercial</h2>

        <form onSubmit={handleSubmit} className="space-y-8">
          
          <div className="bg-white p-4 rounded-lg border-2 border-dashed border-blue-300 flex flex-col md:flex-row items-center gap-6 justify-between">
            <div>
              <label className="block text-sm font-bold text-blue-800 mb-2">Añadir Logo de tu Empresa (Opcional)</label>
              ...
              <input 
                ...
                className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-bold file:bg-blue-100 file:text-blue-700 hover:file:bg-blue-200 cursor-pointer" 
              />
            </div>
            {logoBase64 && (
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div>
              ...
              <input type="text" className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ej. PO-2024-4829" value={numeroPedido} onChange={(e) => setNumeroPedido(e.target.value)} required />
            </div>
            <div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-blue-50 p-5 rounded-lg border border-blue-200 shadow-sm">
              <h4 className="font-bold text-blue-700 mb-4 border-b border-blue-200 pb-2">Datos del Exportador (Tú)</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Razón Social</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={expRazonSocial} onChange={(e) => setExpRazonSocial(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">NIT / Tax ID</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={expNit} onChange={(e) => setExpNit(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Ciudad y País</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={expCiudadPais} onChange={(e) => setExpCiudadPais(e.target.value)} required />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Dirección Física</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={expDireccion} onChange={(e) => setExpDireccion(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Email</label>
                  <input type="email" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={expEmail} onChange={(e) => setExpEmail(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Teléfono</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={expTelefono} onChange={(e) => setExpTelefono(e.target.value)} required />
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Razón Social</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={impRazonSocial} onChange={(e) => setImpRazonSocial(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">NIT</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={impNit} onChange={(e) => setImpNit(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Ciudad y País</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={impCiudadPais} onChange={(e) => setImpCiudadPais(e.target.value)} required />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Dirección Física</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={impDireccion} onChange={(e) => setImpDireccion(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Email</label>
                  <input type="email" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={impEmail} onChange={(e) => setImpEmail(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Teléfono</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-blue-500 outline-none" value={impTelefono} onChange={(e) => setImpTelefono(e.target.value)} required />
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-5 rounded-lg border-2 border-dashed border-blue-300 shadow-inner">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 border-b border-blue-200 pb-3 gap-4">
              <h3 className="text-lg font-bold text-blue-800">Costos Logísticos (Según Incoterm)</h3>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
                ...
                <input 
                  type="text" 
                  className="border border-gray-400 rounded-md p-2 text-sm focus:ring-2 focus:ring-blue-500 bg-white w-full sm:w-auto" 
          ...
          <div>
            ...
            <input type="text" className="w-full border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Ej. 30% Anticipo, 70% contra entrega de documentos" value={condicionesPago} onChange={(e) => setCondicionesPago(e.target.value)} required />
          </div>

          <button type="submit" className="w-full bg-blue-600 text-white font-bold py-4 px-4 rounded-md hover:bg-blue-700 transition-colors shadow-md mt-6 text-lg">
            Descargar Factura Comercial (PDF)
          </button>

          <Link href="/lista-empaque" className="w-full block text-center bg-gray-200 text-gray-700 font-bold py-4 px-4 rounded-md hover:bg-gray-300 transition-colors mt-4 text-lg">
