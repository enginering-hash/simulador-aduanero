...
export default function FacturaProforma() {
  ...
  useEffect(() => {
    ...
    if (!profActual) {
      ...
      profActual = `PROF-${año}-${Math.floor(1000 + Math.random() * 9000)}`;
    }
  ...
  useEffect(() => {
    if (numeroProforma) {
      const borradorActual = {
        numeroProforma, logoBase64, numeroPedido, fechaElaboracion, moneda,
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const columnas = ["CANT.", "DESCRIPCIÓN", `PRECIO UNITARIO (${moneda})`, `IMPORTE (${moneda})`];
    ...
    doc.save(`Factura_Proforma_${numeroProforma}.pdf`);
    localStorage.setItem('numeroProforma', numeroProforma);
  };
    ...
    <main className="min-h-screen bg-gray-100 p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-5xl mb-4 flex justify-between items-center">
        <button 
          onClick={() => router.back()} 
          type="button"
          className="bg-white text-orange-600 border border-orange-600 px-4 py-2 rounded font-bold hover:bg-orange-50 transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="w-full max-w-5xl bg-white rounded-xl shadow-lg p-8 border-t-8 border-orange-500">
        
        <div className="bg-orange-50 border-l-4 border-orange-500 p-4 mb-8 rounded-r-lg">
          <h3 className="font-bold text-orange-800 text-lg mb-1">Paso 2: Factura Proforma</h3>
          <p className="text-orange-700 text-sm">
            <strong>Tu Rol:</strong> Exportador / Vendedor.<br/>
            <strong>Objetivo:</strong> Responder a la Orden de Compra detallando los costos logísticos según el Incoterm seleccionado.
          </p>
        </div>

        <h2 className="text-3xl font-bold text-center text-orange-600 mb-8">Factura Proforma</h2>

        <form onSubmit={handleSubmit} className="space-y-8">
          
          <div className="bg-white p-4 rounded-lg border-2 border-dashed border-orange-300 flex flex-col md:flex-row items-center gap-6 justify-between">
            <div>
              <label className="block text-sm font-bold text-orange-800 mb-2">Añadir Logo de tu Empresa (Opcional)</label>
              ...
              <input 
                ...
                className="text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-bold file:bg-orange-100 file:text-orange-700 hover:file:bg-orange-200 cursor-pointer" 
              />
            </div>
            {logoBase64 && (
          ...
          {/* AJUSTADO A 4 COLUMNAS PARA INCLUIR LA MONEDA */}
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">N° de Pedido (PO)</label>
              <input type="text" className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-orange-500 outline-none" placeholder="Ej. PO-2024-4829" value={numeroPedido} onChange={(e) => setNumeroPedido(e.target.value)} required />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Fecha de Elaboración</label>
              <input type="date" className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-orange-500 outline-none" value={fechaElaboracion} onChange={(e) => setFechaElaboracion(e.target.value)} required />
            </div>
            <div>
              <label className="block text-xs font-bold text-gray-700 mb-1">Moneda</label>
              <select className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-orange-500 outline-none bg-white font-bold" value={moneda} onChange={(e) => setMoneda(e.target.value)} required>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-orange-50 p-5 rounded-lg border border-orange-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  ...
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={expRazonSocial} onChange={(e) => setExpRazonSocial(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">NIT / Tax ID</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={expNit} onChange={(e) => setExpNit(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Ciudad y País</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={expCiudadPais} onChange={(e) => setExpCiudadPais(e.target.value)} required />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Dirección Física</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={expDireccion} onChange={(e) => setExpDireccion(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Email</label>
                  <input type="email" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={expEmail} onChange={(e) => setExpEmail(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Teléfono</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={expTelefono} onChange={(e) => setExpTelefono(e.target.value)} required />
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Razón Social</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={impRazonSocial} onChange={(e) => setImpRazonSocial(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">NIT</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={impNit} onChange={(e) => setImpNit(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Ciudad y País</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={impCiudadPais} onChange={(e) => setImpCiudadPais(e.target.value)} required />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Dirección Física</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={impDireccion} onChange={(e) => setImpDireccion(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Email</label>
                  <input type="email" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={impEmail} onChange={(e) => setImpEmail(e.target.value)} required />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Teléfono</label>
                  <input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-orange-500 outline-none" value={impTelefono} onChange={(e) => setImpTelefono(e.target.value)} required />
                </div>
              </div>
            </div>
          </div>
          ...
          <div>
            ...
            {productos.map((producto, index) => (
              <div key={index} className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-3 items-end relative bg-gray-50 p-4 rounded-lg border border-gray-200">
                <span className="absolute -top-3 -left-3 bg-orange-600 text-white w-8 h-8 flex items-center justify-center rounded-full font-bold shadow-md z-10">
                  {index + 1}
                </span>
                ...
                <div className="md:col-span-1">
                  ...
                  <input type="number" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-orange-500" value={producto.cantidad} onChange={(e) => actualizarProducto(index, 'cantidad', e.target.value)} required />
                </div>
                <div className="md:col-span-2">
                  <label className="block text-xs font-bold text-gray-700 mb-1">DESCRIPCIÓN</label>
                  <input type="text" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-orange-500" value={producto.descripcion} onChange={(e) => actualizarProducto(index, 'descripcion', e.target.value)} required />
                </div>
                <div className="md:col-span-1">
                  <label className="block text-xs font-bold text-gray-700 mb-1">PRECIO UNITARIO ({moneda})</label>
                  <input type="number" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-orange-500" value={producto.precioUnitario} onChange={(e) => actualizarProducto(index, 'precioUnitario', e.target.value)} required />
                </div>
              </div>
            ))}
          ...
          <div className="bg-white p-5 rounded-lg border-2 border-dashed border-orange-300 shadow-inner">
            <div className="flex flex-col md:flex-row md:items-center justify-between mb-4 border-b border-orange-200 pb-3 gap-4">
              <h3 className="text-lg font-bold text-orange-800">Costos Logísticos (Según Incoterm)</h3>
              ...
              <div className="flex flex-col sm:flex-row items-center gap-2 w-full md:w-auto">
                ...
                <input 
                  type="text" 
                  className="border border-gray-400 rounded-md p-2 text-sm focus:ring-2 focus:ring-orange-500 bg-white w-full sm:w-auto" 
              ...
              {incoterm !== "EXW" && (
                <>
                  <div>
                    ...
                    <input type="number" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-orange-500" value={fleteNacionalOrigen} onChange={(e) => setFleteNacionalOrigen(e.target.value)} />
                  </div>
                  <div>
                    <label className="block text-xs font-bold text-gray-700 mb-1">Gastos Exportación ({moneda})</label>
                    <input type="number" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-orange-500" value={gastosExportacion} onChange={(e) => setGastosExportacion(e.target.value)} />
                  </div>
                </>
              )}
          ...
          <div>
            ...
            <input type="text" className="w-full border border-gray-300 rounded-md p-3 focus:ring-2 focus:ring-orange-500 outline-none" placeholder="Ej. 30% Anticipo, 70% contra entrega de documentos" value={condicionesPago} onChange={(e) => setCondicionesPago(e.target.value)} required />
          </div>

          <button type="submit" className="w-full bg-orange-600 text-white font-bold py-4 px-4 rounded-md hover:bg-orange-700 transition-colors shadow-md mt-6 text-lg">
            Descargar Factura Proforma (PDF)
          </button>

          <Link href="/aprobacion-correo" className="w-full block text-center bg-gray-200 text-gray-700 font-bold py-4 px-4 rounded-md hover:bg-gray-300 transition-colors mt-4 text-lg">
