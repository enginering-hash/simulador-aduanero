...
export default function AprobacionReservaExportacion() {
  ...
  useEffect(() => {
    // Usar la memoria específica de exportación
    const borradorGuardado = sessionStorage.getItem("borrador_aprobacion_reserva_expo_v1");
    ...
    if (borradorGuardado) {
      ...
      // Heredar datos del "Formato de Reserva" de Exportación
  ...
  useEffect(() => {
    if (bookingRef) {
      ...
      sessionStorage.setItem("borrador_aprobacion_reserva_expo_v1", JSON.stringify(borradorActual));
      
      const globalActual = localStorage.getItem('datosReservaExpo');
      if(globalActual){
        ...
        localStorage.setItem('datosReservaExpo', JSON.stringify(nuevosGlobales));
      }
    }
  });
  ...
  const generarPDF = (e: React.FormEvent) => {
    ...
    doc.setFontSize(22);
    doc.setTextColor(109, 40, 217); // Violeta Naviera
    ...
    doc.save(`Booking_Confirmation_Exportacion_${bookingRef}.pdf`);
  };
    ...
    <main className="min-h-screen bg-gray-200 p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-4xl mb-4 flex justify-between items-center">
        <button 
          onClick={() => router.back()} 
          type="button"
          className="bg-white text-violet-700 border border-violet-700 px-4 py-2 rounded font-bold hover:bg-violet-50 transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-2xl overflow-hidden border-t-8 border-violet-600">
        
        <div className="bg-violet-900 text-white p-6 flex flex-col gap-4">
          <div className="flex justify-between items-start">
            <div className="w-2/3">
              <input 
                type="text" 
                className="w-full bg-transparent border-b border-violet-400 text-2xl font-black tracking-wider outline-none focus:border-white placeholder-violet-300 mb-2 uppercase"
                value={naviera}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-2 mt-2">
                <input 
                  type="text" 
                  className="bg-violet-800 border border-violet-700 text-xs p-1 rounded outline-none focus:ring-1 focus:ring-violet-300 placeholder-violet-400"
                  value={navieraOffice}
            ...
            <div className="text-right">
              <p className="text-sm font-bold text-violet-200">Estado de Solicitud:</p>
              <p className="bg-green-500 text-white px-3 py-1 rounded-full font-bold text-sm inline-block mt-1 animate-pulse">APROBADO</p>
            </div>
          </div>
          
          <div className="flex items-center gap-4 bg-violet-800/50 p-3 rounded-md border border-violet-700 w-full md:w-2/3">
              {logoBase64 ? (
                  <img src={logoBase64} alt="Logo Naviera" className="w-12 h-12 rounded border border-violet-300 object-cover bg-white p-1" />
              ) : (
                  <div className="w-12 h-12 rounded border border-dashed border-violet-400 bg-violet-800 flex items-center justify-center text-[10px] text-violet-300 text-center leading-tight">Sin Logo</div>
              )}
              <div>
                  <label className="block text-[10px] font-bold text-violet-200 mb-1 uppercase">Logo de la Naviera (Cuadrado):</label>
                  <input type="file" ref={fileInputRef} accept="image/*" onChange={handleLogoUpload} className="text-[10px] text-white file:mr-2 file:py-1 file:px-2 file:rounded file:border-0 file:text-[10px] file:font-semibold file:bg-violet-100 file:text-violet-800 hover:file:bg-violet-200 cursor-pointer" />
                  {logoBase64 && (
        ...
        <div className="p-8">
          <div className="bg-violet-50 border-l-4 border-violet-600 p-4 mb-8">
            <h3 className="font-bold text-violet-900 text-lg mb-1">Paso 5: Recepción del Booking Confirmation (Exportación)</h3>
          ...
          <form onSubmit={generarPDF}>
            <div className="border border-gray-300 p-6 rounded bg-gray-50 mb-8 text-sm text-gray-800 shadow-inner">
              ...
              <div className="flex flex-col md:flex-row justify-center items-center gap-2 mb-6 border-b pb-4">
                <h2 className="font-bold text-lg text-violet-900">BOOKING CONFIRMATION NO.</h2>
                <input 
                  type="text" 
                  className="border border-gray-300 rounded px-2 py-1 font-bold text-lg text-center w-48 outline-none focus:ring-2 focus:ring-violet-500"
                  value={bookingRef}
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                <div className="col-span-2 space-y-3">
                  <div className="flex items-center gap-2">
                    ...
                    <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-violet-500" placeholder="Nombre del Buque (Ej: MSC GALAXY)" value={buque} onChange={(e) => setBuque(e.target.value)} required />
                  </div>
                  <div className="flex items-center gap-2">
                    <strong className="w-32">ETA DESTINO:</strong>
                    <input type="date" className="flex-1 border border-gray-300 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-violet-500" value={eta} onChange={(e) => setEta(e.target.value)} required />
                  </div>
                </div>
                <div>
                  <div className="flex items-center gap-2">
                    <strong className="w-24 leading-tight">FECHA DE ZARPE:</strong>
                    <input type="date" className="flex-1 border border-gray-300 rounded px-2 py-1 outline-none focus:ring-1 focus:ring-violet-500" value={fechaActual} onChange={(e) => setFechaActual(e.target.value)} required />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white p-4 border border-gray-300 grid grid-cols-1 gap-3 mb-6">
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={shipperNombre} onChange={(e) => setShipperNombre(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">CNEE:</strong>
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={consignatarioNombre} onChange={(e) => setConsignatarioNombre(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">PTO. DE EMBARQUE:</strong>
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={puertoCargue} onChange={(e) => setPuertoCargue(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">DESTINO FINAL:</strong>
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={destino} onChange={(e) => setDestino(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">PRODUCTOS:</strong>
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={mercancia} onChange={(e) => setMercancia(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">CONTAINERS:</strong>
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={contenedores} onChange={(e) => setContenedores(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">CONDICION:</strong>
                  <select className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500 bg-white" value={tipoServicio} onChange={(e) => setTipoServicio(e.target.value)} required>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={embarqueVia} onChange={(e) => setEmbarqueVia(e.target.value)} required />
                </div>
                <div className="flex items-center gap-2">
                  <strong className="w-40 text-xs">FLETE MARÍTIMO:</strong>
                  <input type="text" className="flex-1 border border-gray-300 rounded px-2 py-1 text-xs outline-none focus:ring-1 focus:ring-violet-500" value={fleteMaritimo} onChange={(e) => setFleteMaritimo(e.target.value)} required />
                </div>
              </div>
            ...
            <div className="flex flex-col md:flex-row gap-4">
              <button type="submit" className="flex-1 bg-violet-800 text-white font-bold py-4 px-4 rounded-md hover:bg-violet-900 transition-colors shadow-md text-center">
                📥 Descargar Booking Confirmation (PDF)
              </button>

              <Link href="/carta-retiro-exportacion" className="flex-1 bg-gray-200 text-gray-800 font-bold py-4 px-4 rounded-md hover:bg-gray-300 transition-colors shadow-md text-center flex items-center justify-center">
