...
export default function CertificadoLoteExportacion() {
  ...
  useEffect(() => {
    ...
    const borradorGuardado = sessionStorage.getItem("borrador_certificado_lote_expo_v1");
    ...
    if (borradorGuardado) {
      ...
      // Si NO hay borrador, cargamos datos heredados de la reserva de EXPORTACIÓN
      setFechaEmision(hoy);
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("borrador_certificado_lote_expo_v1", JSON.stringify(borradorActual));
  });
  ...
  const generarPDF = (e: React.FormEvent) => {
    ...
    doc.setFontSize(18);
    doc.setTextColor(220, 38, 38); // Rojo oscuro (Red 600)
    ...
    autoTable(doc, {
      ...
      body: filas,
      theme: 'grid',
      headStyles: { fillColor: [239, 68, 68], textColor: 255, fontStyle: 'bold', halign: 'center' }, // Red 500
    ...
    doc.save(`Certificado_Lote_Exportacion_${exportador.replace(/\s+/g, '_')}.pdf`);
  };
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-4 md:p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-6xl mb-4 flex justify-between items-center">
        <button onClick={() => router.back()} type="button" className="bg-white text-red-700 border border-red-700 px-4 py-2 rounded font-bold hover:bg-red-50 transition-colors flex items-center gap-2 shadow-sm">
      ...
      <div className="w-full max-w-6xl bg-white rounded-xl shadow-2xl border-t-8 border-red-600 overflow-hidden">
        
        <div className="bg-red-50 p-6 border-b border-red-200">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-red-600 text-white rounded-full flex items-center justify-center font-black text-xl shadow-md">QA</div>
            <div>
              <h2 className="font-black text-2xl text-red-900 uppercase tracking-tight">Certificado de Lote (COA) - Exportación</h2>
        ...
        <form onSubmit={generarPDF} className="p-8 space-y-8">
          ...
          <section className="bg-white p-6 rounded-lg border border-gray-300 shadow-sm">
            <h3 className="font-bold text-red-800 mb-4 border-b-2 border-red-100 pb-2">1. DATOS GENERALES</h3>
            
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-4">
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Fecha de Emisión:</label>
                <input type="date" className="w-full border p-2 rounded focus:ring-2 focus:ring-red-500 outline-none" value={fechaEmision} onChange={(e)=>setFechaEmision(e.target.value)} required />
              </div>
              <div className="md:col-span-2">
                <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Empresa Exportadora (Fabricante):</label>
                <input type="text" className="w-full border p-2 rounded focus:ring-2 focus:ring-red-500 outline-none font-bold" value={exportador} onChange={(e)=>setExportador(e.target.value)} required />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" className="w-full border p-2 rounded focus:ring-2 focus:ring-red-500 outline-none" value={consignatario} onChange={(e)=>setConsignatario(e.target.value)} required />
              </div>
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Director Técnico (Quien firma):</label>
                <input type="text" className="w-full border p-2 rounded focus:ring-2 focus:ring-red-500 outline-none" placeholder="Ej. Ing. Laura Gómez" value={directorTecnico} onChange={(e)=>setDirectorTecnico(e.target.value)} required />
              </div>
            </div>
          </section>
          ...
          <section>
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-lg font-bold text-red-900 border-b border-red-200 pb-2">2. TRAZABILIDAD DE LOTES</h3>
              <button type="button" onClick={agregarLote} className="bg-red-100 text-red-800 font-bold py-2 px-4 rounded-md hover:bg-red-200 text-sm shadow-sm transition-colors border border-red-300">
                + Agregar Lote
              </button>
            </div>
            ...
            <div className="flex items-center gap-2 mb-2">
              <div className="hidden md:grid grid-cols-12 gap-2 bg-red-600 text-white p-2 rounded-md text-[10px] font-bold text-center items-center w-full shadow-sm">
              ...
              <div key={index} className="flex flex-col md:flex-row items-start md:items-center gap-2 mb-2">
                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 border border-gray-300 bg-white p-4 md:p-2 rounded-md shadow-sm w-full">
                  ...
                  <div className="md:col-span-1">
                    ...
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500" placeholder="8710" value={lote.codigo} onChange={(e) => actualizarLote(index, 'codigo', e.target.value)} required />
                  </div>
                  
                  <div className="md:col-span-3">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Nombre:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm outline-none focus:ring-1 focus:ring-red-500" placeholder="Galleta de leche..." value={lote.nombre} onChange={(e) => actualizarLote(index, 'nombre', e.target.value)} required />
                  </div>
                  
                  <div className="md:col-span-1">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Cantidad:</label>
                    <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500" placeholder="135" value={lote.cantidad} onChange={(e) => actualizarLote(index, 'cantidad', e.target.value)} required />
                  </div>
                  
                  <div className="md:col-span-1">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">P. Neto:</label>
                    <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500" placeholder="931.5" value={lote.pesoNeto} onChange={(e) => actualizarLote(index, 'pesoNeto', e.target.value)} required />
                  </div>
                  
                  <div className="md:col-span-1">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Lote:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500 font-bold text-red-700" placeholder="774839-1" value={lote.lote} onChange={(e) => actualizarLote(index, 'lote', e.target.value)} required />
                  </div>

                  <div className="md:col-span-2">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Fecha Prod:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500" placeholder="Ej. 30-sep-25" value={lote.fechaProduccion} onChange={(e) => actualizarLote(index, 'fechaProduccion', e.target.value)} required />
                  </div>

                  <div className="md:col-span-2">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Fecha Venc:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500" placeholder="Ej. 30-sep-26" value={lote.fechaVencimiento} onChange={(e) => actualizarLote(index, 'fechaVencimiento', e.target.value)} required />
                  </div>

                  <div className="md:col-span-1">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Turno:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-red-500" placeholder="Mañana" value={lote.turno} onChange={(e) => actualizarLote(index, 'turno', e.target.value)} required />
                  </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 pt-4">
            <button type="submit" className="flex-1 bg-red-600 text-white font-bold py-4 rounded shadow-lg hover:bg-red-700 transition-all text-lg border-b-4 border-red-800">
              📥 Descargar Certificado de Lote (PDF)
            </button>
            <Link href="/carta-responsabilidad" className="flex-1 bg-red-100 text-red-800 font-bold py-4 rounded shadow-lg hover:bg-red-200 text-center text-lg border-b-4 border-red-300 flex items-center justify-center">
              Siguiente Paso: Carta de Responsabilidad →
            </Link>
          </div>
