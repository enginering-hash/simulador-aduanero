...
export default function CertificadoInvimaExportacion() {
  ...
  const [tipoImportacion, setTipoImportacion] = useState("Exportación"); // Modificado para Expo
  ...
  const [observaciones, setObservaciones] = useState(
    "Los productos alimenticios y las materias primas de exportación cumplen con los requisitos establecidos en la normativa sanitaria vigente en Colombia para su salida del país.\n\nEn caso de presentarse deficiencias en el rotulado, el exportador deberá ajustarse a lo dispuesto en la Resolución 5109 de 2005 o en la normativa que la sustituya o complemente."
  );

  // CARGAR LOS DATOS AL ABRIR LA PÁGINA
  useEffect(() => {
    // Usar la memoria de exportación
    const borradorGuardado = sessionStorage.getItem("borrador_certificado_invima_expo_v1");
    ...
    if (borradorGuardado) {
      ...
      setTipoImportacion(datos.tipoImportacion || "Exportación");
      ...
      // Heredar datos de la reserva de exportación
  ...
  useEffect(() => {
    ...
    sessionStorage.setItem("borrador_certificado_invima_expo_v1", JSON.stringify(borradorActual));
  });
  ...
  const generarContenidoPDF = (doc: jsPDF) => {
    ...
    doc.setFont("helvetica", "bold");
    doc.text("Tipo Operación:", 150, 40); doc.setFont("helvetica", "normal"); doc.text(tipoImportacion, 182, 40);
    ...
    doc.setFontSize(9);
    doc.text("Identificación (NIT/ID):", 30, 90); doc.setFont("helvetica", "normal"); doc.text(identificacion, 65, 90);
    doc.setFont("helvetica", "bold");
    doc.text("Ciudad / País:", 150, 90); doc.setFont("helvetica", "normal"); doc.text(municipio, 175, 90);
    ...
    doc.save(`Acta_INVIMA_Exportacion_${radicado}.pdf`);
  };
    ...
    <main className="min-h-screen bg-gray-100 p-4 md:p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-6xl bg-white rounded-xl shadow-2xl border-t-8 border-sky-600 overflow-hidden">
        ...
        <div className="bg-sky-50 p-6 border-b border-sky-200">
            <h2 className="font-black text-2xl text-sky-900 uppercase tracking-tight">Certificado de Inspección (INVIMA) - Exportación</h2>
            <p className="text-sm text-sky-700 font-medium">Acta de inspección sanitaria para salida de productos del país.</p>
        </div>
        ...
        <form onSubmit={generarPDF} className="p-8 space-y-6">
          ...
          <section className="bg-white p-5 rounded-lg border border-gray-300 shadow-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div><label className="block text-[11px] font-bold text-gray-600 mb-1 uppercase">Tipo Operación:</label><input type="text" className="w-full border p-2 text-sm rounded outline-none focus:ring-1 focus:ring-sky-500" value={tipoImportacion} onChange={(e)=>setTipoImportacion(e.target.value)} required /></div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              ...
              <div><label className="block text-[11px] font-bold text-gray-600 mb-1 uppercase">Doc. Transporte (Booking):</label><input type="text" className="w-full border p-2 text-sm rounded outline-none focus:ring-1 focus:ring-sky-500" value={docTransporte} onChange={(e)=>setDocTransporte(e.target.value)} required /></div>
            </div>
          ...
          <section className="bg-sky-50/50 p-5 rounded-lg border border-sky-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div><label className="block text-[11px] font-bold text-gray-600 mb-1 uppercase">Identificación (NIT/ID):</label><input type="text" className="w-full border p-2 text-sm rounded outline-none focus:ring-1 focus:ring-sky-500" value={identificacion} onChange={(e)=>setIdentificacion(e.target.value)} required /></div>
              <div><label className="block text-[11px] font-bold text-gray-600 mb-1 uppercase">Ciudad / País:</label><input type="text" className="w-full border p-2 text-sm rounded outline-none focus:ring-1 focus:ring-sky-500" placeholder="Ej. MIAMI - USA" value={municipio} onChange={(e)=>setMunicipio(e.target.value)} required /></div>
            </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 pt-4">
            ...
            <Link href="/documento-transporte-exportacion" className="flex-1 bg-blue-700 text-white font-bold py-4 rounded shadow-lg hover:bg-blue-800 text-center text-lg border-b-4 border-blue-900 flex items-center justify-center">
