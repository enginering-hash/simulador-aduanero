...
export default function ListaEmpaqueExportacion() {
  ...
  useEffect(() => {
    ...
    const borradorGuardado = sessionStorage.getItem("borrador_lista_empaque_expo_v1"); 
    
    // Heredar de Factura Comercial de EXPORTACIÓN
    const datosFactura = sessionStorage.getItem("borrador_factura_comercial_expo_v1");
    ...
    if (borradorGuardado) {
      ...
      if(fData) {
        ...
        setFormaPago(fData.condicionesPago || "");
        
        // Auto-completar los productos desde la factura con campos extra vacíos
        if (fData.productos && fData.productos.length > 0) {
          const prodsAdapter = fData.productos.map((p: any) => ({
            referencia: "",
    ...
    if (!plActual) {
      ...
      plActual = `PL-EXP-${Math.floor(100 + Math.random() * 900)}/${año}`;
    }
  ...
  useEffect(() => {
    if (numeroLista) {
      ...
      sessionStorage.setItem("borrador_lista_empaque_expo_v1", JSON.stringify(borradorActual));
    }
  });
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    doc.save(`Packing_List_Exportacion_${numeroLista.replace('/', '-')}.pdf`);
  };
  ...
  return (
    <main className="min-h-screen bg-gray-100 p-8 flex flex-col items-center">
      ...
      <div className="w-full max-w-6xl bg-white rounded-xl shadow-lg p-8 border-t-8 border-amber-600">
        
        <div className="bg-amber-50 border-l-4 border-amber-600 p-4 mb-8 rounded-r-lg">
          <h3 className="font-bold text-amber-800 text-lg mb-1">Paso 8: Lista de Empaque (Exportación)</h3>
          <p className="text-amber-700 text-sm">
            Detalla la organización física de la carga. Este documento es clave para el transporte y para la revisión de aduanas en el país destino.
          </p>
        </div>

        <h2 className="text-3xl font-bold text-center text-amber-700 mb-8 uppercase">Lista de Empaque</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-gray-50 p-4 rounded-lg border border-gray-200">
            ...
            <div>
              ...
              <input type="date" className="w-full border border-gray-300 rounded-md p-2 focus:ring-2 focus:ring-amber-500 outline-none bg-white" value={fechaEmision} onChange={(e) => setFechaEmision(e.target.value)} required />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-amber-50 p-5 rounded-lg border border-amber-200 shadow-sm">
              <h4 className="font-bold text-amber-700 mb-4 border-b border-amber-200 pb-2">Datos del Exportador (Tú)</h4>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2"><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Razón Social</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={expRazonSocial} onChange={(e) => setExpRazonSocial(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">NIT / Tax ID</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={expNit} onChange={(e) => setExpNit(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Ciudad y País</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={expCiudadPais} onChange={(e) => setExpCiudadPais(e.target.value)} required /></div>
                <div className="sm:col-span-2"><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Dirección Física</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={expDireccion} onChange={(e) => setExpDireccion(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Email</label><input type="email" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={expEmail} onChange={(e) => setExpEmail(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Teléfono</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={expTelefono} onChange={(e) => setExpTelefono(e.target.value)} required /></div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-5 rounded-lg border border-gray-200 shadow-sm">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div className="sm:col-span-2"><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Razón Social</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={impRazonSocial} onChange={(e) => setImpRazonSocial(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">NIT</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={impNit} onChange={(e) => setImpNit(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Ciudad y País</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={impCiudadPais} onChange={(e) => setImpCiudadPais(e.target.value)} required /></div>
                <div className="sm:col-span-2"><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Dirección Física</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={impDireccion} onChange={(e) => setImpDireccion(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Email</label><input type="email" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={impEmail} onChange={(e) => setImpEmail(e.target.value)} required /></div>
                <div><label className="block text-[10px] font-bold text-gray-700 mb-1 uppercase">Teléfono</label><input type="text" className="w-full border border-gray-300 rounded p-2 text-xs focus:ring-1 focus:ring-amber-500 outline-none" value={impTelefono} onChange={(e) => setImpTelefono(e.target.value)} required /></div>
              </div>
            </div>
          </div>

          <div className="bg-white p-5 rounded-lg border-2 border-dashed border-amber-300 shadow-inner">
            <h3 className="text-lg font-bold text-amber-800 mb-4 border-b border-amber-200 pb-2">Condiciones de Embarque</h3>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <div className="md:col-span-1"><label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Incoterm:</label><input type="text" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-amber-500" value={incoterm} onChange={(e) => setIncoterm(e.target.value)} required /></div>
              <div className="md:col-span-1"><label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Forma de Pago:</label><input type="text" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-amber-500" value={formaPago} onChange={(e) => setFormaPago(e.target.value)} required /></div>
              <div className="md:col-span-1"><label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Puerto Origen:</label><input type="text" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-amber-500" value={puertoEmbarque} onChange={(e) => setPuertoEmbarque(e.target.value)} required /></div>
              <div className="md:col-span-1"><label className="block text-xs font-bold text-gray-700 mb-1 uppercase">Puerto Destino:</label><input type="text" className="w-full border border-gray-300 rounded-md p-2 text-sm focus:ring-2 focus:ring-amber-500" value={puertoDestino} onChange={(e) => setPuertoDestino(e.target.value)} required /></div>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2 mb-2">
              <div className="hidden md:grid grid-cols-11 gap-2 bg-amber-600 text-white p-2 rounded-md text-[10px] font-bold text-center items-center w-full shadow-sm">
              ...
              <div key={index} className="flex flex-col md:flex-row items-start md:items-center gap-2 mb-2">
                ...
                <div className="grid grid-cols-1 md:grid-cols-11 gap-2 border border-gray-300 bg-white p-4 md:p-2 rounded-md shadow-sm w-full">
                  <div className="md:col-span-1">
                    ...
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500" placeholder="REF-01" value={producto.referencia} onChange={(e) => actualizarProducto(index, 'referencia', e.target.value)} required />
                  </div>
                  
                  <div className="md:col-span-3">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Descripción:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm outline-none focus:ring-1 focus:ring-amber-500" placeholder="Ej. Partes de motor" value={producto.descripcion} onChange={(e) => actualizarProducto(index, 'descripcion', e.target.value)} required />
                  </div>
                  
                  <div className="md:col-span-2">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Subpartida:</label>
                    <input type="text" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500" placeholder="8409.91.00" value={producto.subpartida} onChange={(e) => actualizarProducto(index, 'subpartida', e.target.value)} required />
                  </div>
                  ...
                  <div className="grid grid-cols-2 md:col-span-2 gap-2">
                    <div>
                      ...
                      <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500" placeholder="100" value={producto.pesoBruto} onChange={(e) => actualizarProducto(index, 'pesoBruto', e.target.value)} required />
                    </div>
                    <div>
                      <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">P. Neto:</label>
                      <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500" placeholder="95" value={producto.pesoNeto} onChange={(e) => actualizarProducto(index, 'pesoNeto', e.target.value)} required />
                    </div>
                  </div>

                  <div className="md:col-span-1">
                    <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Cajas:</label>
                    <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500" placeholder="50" value={producto.cajas} onChange={(e) => actualizarProducto(index, 'cajas', e.target.value)} required />
                  </div>

                  <div className="grid grid-cols-2 md:col-span-2 gap-2">
                    <div>
                      <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">m³ / Caja:</label>
                      <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500" placeholder="0.25" value={producto.m3PorCaja} onChange={(e) => actualizarProducto(index, 'm3PorCaja', e.target.value)} required />
                    </div>
                    <div>
                      <label className="md:hidden block text-[10px] font-bold text-gray-500 mb-1 uppercase">Total m³:</label>
                      <input type="number" className="w-full border rounded p-1 text-sm text-center outline-none focus:ring-1 focus:ring-amber-500 bg-gray-50" placeholder="12.5" value={producto.totalM3} onChange={(e) => actualizarProducto(index, 'totalM3', e.target.value)} required />
                    </div>
                  </div>
                </div>
          ...
          <div>
            ...
            <textarea className="w-full border border-gray-300 p-3 text-sm rounded-md bg-white outline-none resize-none h-20 focus:ring-2 focus:ring-amber-500" placeholder="Notas adicionales del empaque (opcional)..." value={observaciones} onChange={(e)=>setObservaciones(e.target.value)} />
          </div>

          <button type="submit" className="w-full bg-amber-600 text-white font-bold py-4 px-4 rounded-md hover:bg-amber-700 transition-colors shadow-md mt-6 text-lg">
            Descargar Packing List (PDF)
          </button>

          <Link href="/certificado-origen-exportacion" className="w-full block text-center bg-amber-100 text-amber-800 font-bold py-4 px-4 rounded-md hover:bg-amber-200 transition-colors mt-4 text-lg">
